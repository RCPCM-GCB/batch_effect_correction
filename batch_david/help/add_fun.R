...
Scatter_Density <- function(data = data, batch = batch, trt = NULL, expl.var = expl.var,
  ...
  trt = as.factor(trt)
  if(nlevels(trt) >= 2){
    pMain <- ggplot(data = data, aes(x = data[ ,1], y = data[ ,2], colour = trt, shape = batch)) + 
      ...
      scale_color_brewer(palette="Dark2") + theme_bw() + xlim(xlim[1], xlim[2]) + 
    ...
    pTop <- ggplot(data,aes(x = data[ ,1], fill = batch, linetype = trt)) + 
      geom_density(size = density.lwd, alpha = 0.2) + ylab('Density') + 
    ...
    pRight <- ggplot(data, aes(x=data[ ,2], fill = batch, linetype = trt)) + 
      geom_density(size = density.lwd,alpha = 0.2) +  coord_flip() + ylab('Density') +
